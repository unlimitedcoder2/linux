...
void check_latency(lkl_long_t (*f)(void), lkl_long_t *min, lkl_long_t *max, lkl_long_t *avg)
{
	int i;
...
static lkl_long_t native_getpid(void)
{
#ifdef __MINGW32__
	GetCurrentProcessId();
#else
...
static int lkl_test_shared_mmap(void)
{
	...
	if (lkl_sys_munmap((lkl_ulong_t)mem1, mem_size) != 0)
		return TEST_FAILURE;

	if (lkl_sys_munmap((lkl_ulong_t)mem2, mem_size) != 0)
		return TEST_FAILURE;
...
static int lkl_test_private_mmap(void)
{
	...
	*(unsigned int *)mem = 13;
	...
	if (lkl_sys_munmap((lkl_ulong_t)mem, mem_size) < 0)
		return TEST_FAILURE;
